...
class moduleClass:
    def __init__(self, classID, world, collectable = True,  weight = 1, unit_reward = 1, gamma = 0.9, random_gen = True, max_inst = 5):
        ...
        if (not random_gen):
            ...
            if self.unit_reward == 1: # rewarding objects must be collected
                ...
                self.gamma = round(random.uniform(0,0.1),2)

        # construct a list of module instances that belong to the same module class
        num_inst = random.randint(1, self.max_inst)
