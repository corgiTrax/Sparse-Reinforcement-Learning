...
class FeatureGridMDP: public GridMDP{
	...
	private:
		...
		vector<pair<float,float>> state_centers;
		float state0_x = -3.2;
		float state0_y = 2.4;
		...
		FeatureGridMDP(unsigned int width, unsigned int height, bool* initStates, bool* termStates, unsigned int nFeatures, double* fWeights, double** sFeatures, double gamma=0.95): GridMDP(width, height, initStates, termStates, gamma), numFeatures(nFeatures)
	{
		...
		FeatureGridMDP(unsigned int width, unsigned int height, vector<unsigned int> initStates, vector<unsigned int> termStates, unsigned int nFeatures, double* fWeights, vector<pair<float,float>> &in_targets,vector<pair<float,float>> &in_obstacles,vector<pair<float,float>> &in_path, double gamma=0.95): GridMDP(width, height, initStates, termStates, gamma), numFeatures(nFeatures)
	{
		featureWeights = new double[numFeatures];
		for(unsigned int i=0; i<numFeatures; i++)
			featureWeights[i] = fWeights[i];
		...
		{   
			stateFeatures[i] = new double[numFeatures];
			state_centers.push_back(make_pair(i % GRID_LENGTH * GRID_CELL_SIZE + state0_x,- (i / GRID_LENGTH) * GRID_CELL_SIZE + state0_y));
		}

		for(pair<float,float> target: in_targets) targets.push_back(target);
		for(pair<float,float> obstacle: in_obstacles) obstacles.push_back(obstacle);
		for(pair<float,float> pathpoint: in_path) path.push_back(pathpoint);
		updateFeatures();
		computeCachedRewards();
	};

		FeatureGridMDP(unsigned int width, unsigned int height, vector<unsigned int> initStates, vector<unsigned int> termStates, unsigned int nFeatures, double* fWeights, string feature_file, double gamma=0.95): GridMDP(width, height, initStates, termStates, gamma), numFeatures(nFeatures)
	{
		featureWeights = new double[numFeatures];
		for(unsigned int i=0; i<numFeatures; i++)
			featureWeights[i] = fWeights[i];
		...
		if(f_file.is_open())
		{
			...
			{
				line_ct += 1;
				vector<string> results0;
				vector<string> results1;
				split(line, ':', results0);
				split(results0[1],'(',results1);
				for(string point: results1)
				{
					if(point.size() < 2) continue;
					point.erase(remove(point.begin(), point.end(),','), point.end());
					point.erase(remove(point.begin(), point.end(),')'), point.end());
					vector<string> xy;
					split(point,' ',xy);
		...
		FeatureGridMDP* deepcopy(){
			...
			return copy;
                ...
                bool updateObjects(pair<unsigned int,unsigned int> demo){

                        bool updated = false;
                        unsigned int next_state = getNextState(demo.first,demo.second);
                        vector<unsigned int> to_delete;
                        unsigned int index = 0;

			//check targets
                        for(pair<float,float> target: targets)
			{
			...
			for(unsigned int i: to_delete)
                        {
				targets.erase(targets.begin()+i-removed_ct);
                                removed_ct++;
			}
			to_delete.clear();
			...
			for(unsigned int i: to_delete)
                        {
				obstacles.erase(obstacles.begin()+i-removed_ct);
                                removed_ct++;
			}
			to_delete.clear();
			...
			for(unsigned int i: to_delete)
                        {
				path.erase(path.begin()+i-removed_ct);
                                removed_ct++;
			}
			updateFeatures();
		        computeCachedRewards();
                        return updated; // no updates
                };

		void updateFeatures(){
                        for(unsigned int i=0; i<numStates; i++)
			{
                                for(unsigned int j=0; j<3; j++) stateFeatures[i][j] = 0.0;
				stateFeatures[i][3] = 1.0; 
			}
			for(pair<float,float> target: targets)
			{
				for(unsigned int i=0; i<numStates; i++)
